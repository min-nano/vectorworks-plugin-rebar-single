...
- **断面記号は原点固定ではなく、鉄筋の断面位置に置く**。ただし断面 2D コンポーネントの紙面座標系の原点は **PIO のローカル座標原点＝パスの第 1 頂点**（`CreateCustomObjectPath` はパスの第 1 頂点をプラグイン座標空間の原点へ置く）であるため、記号は**第 1 頂点を基準とした相対位置（重心 − 第 1 頂点）**を各断面の紙面へ投影して置く（front_back は (X, Z)・left_right は (Y, Z)）。**絶対座標（重心そのもの）で置くと第 1 頂点ぶん二重にずれ、本体から大きく離れて描画される（VW 上で観測）。** 3D パス編集画面での位置とデザインレイヤの描画位置に差があると原点固定では記号がその差だけずれるが、第 1 頂点基準の相対配置にすることで本体と一致させる。左右ビューの鏡像（left_right の u=Y の符号）は VW 上で最終確認する。
