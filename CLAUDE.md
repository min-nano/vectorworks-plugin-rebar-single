...
2. **平面（Top/Plan）**: 上から見たパスの投影図（面内の鉄筋＝2D 線）と、呼び径の**2D 表示記号**（縦筋のように上から見ると点になる鉄筋を ●／× で示す）。ハイブリッド図形は Top/Plan で 2D regen を表示するため、3D の端部投影（カギ状）ではなくこのクリーンな 2D 記号が出る。平面の 2D 記号・投影線は PIO 本体クラス。
