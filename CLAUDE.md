...
2. **平面（Top/Plan）**: 上から見たパスの投影図（2D 線）。
3. **断面記号ソリッド（3D）**: 呼び径に応じた表示記号（●／× 等、配筋標準図 KSE 2008「鉄筋の表示記号」）の**断面形状**をパスに沿って押し出したソリッド。断面ビューポートはこの 3D ソリッドを**ネイティブに切断**するため、折れ・フック・斜め配置でも、どの位置で切っても正しい位置に記号が出る。
...
命令セットのスキーマ（version・path/tube_diameter/plan_lines/symbol_profiles の各形式）は `document.py` の docstring に定義されている。`symbol_profiles` は断面（パスに直交する紙面）上の塗り形状（`disk`＝塗り円・`ring`＝中空リング・`polygon`＝塗り多角形）で、原点(0,0)中心に組み立てる（押し出しがパスに沿って配置するため位置合わせ不要）。スキーマを変更するときは `DOCUMENT_VERSION` の互換性に注意し、`TypedDict` 定義・docstring・`validate_document()` とテストも併せて更新すること。`run()` は両フェーズの間で `json.dumps`/`json.loads` を通すため、命令セットに直列化不能なオブジェクト（vs ハンドル等）を入れてはならない。
...
- **断面記号ソリッド**: プロファイルは記号ごとの塗り形状。
  - `disk`（●・中心点）: `vs.Oval` の塗り円を押し出す。
  - `polygon`（× 等の帯）: `vs.Poly` の塗り多角形（長方形）を押し出す。
  - `ring`（○ 等の中空記号）: 外側の円柱ソリッドから内側の円柱ソリッドを `vs.SubtractSolid` で引いて中空にする（実断面が塗り環＝輪郭に見える）。
- 記号ソリッドは押し出しに失敗しても（本体と違い）フォールバックせず描かない。
- **プロファイルの向き・原点、押し出しの生成可否・後始末、記号の紙面上の向き（押し出しの回転フレーム）、左右ビューの見え方は VectorWorks 上で最終確認する**（描画フェーズは VW 上で検証する方針）。
...
- 配筋標準図（KSE 2008）「鉄筋の表示記号」に従い、呼び径ごとに断面の表示記号を塗り形状のプロファイル（disk / ring / polygon）へ分解する（D10=●, D13=×, D16=⊘, D19=●, D22=○, D25=⊙, D29=⊗, D32=◎, D35=⊕, D38=●⊕, D41=⊗）。
- 記号の大きさは `呼び径 × MarkScale` を外径とした模式表現。3D 本体の断面円（最外径）とは別に扱う。○ 等の中空記号はリング（中空）で表す（壁を薄くして輪郭に見せる）。表にない呼び径は最も近い標準呼び径の記号で近似する。
