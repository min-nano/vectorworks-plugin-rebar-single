        ...
        "cut_marks": [
            {
                ...
                # なく、鉄筋の断面位置へ平行移動して置く。断面 2D コンポーネント
                # の紙面座標系の原点は PIO のローカル原点=パスの第 1 頂点
                # (CreateCustomObjectPath がパスの第 1 頂点をプラグイン座標空間の
                # 原点へ置く)であるため、記号は**第 1 頂点を基準とした相対位置**
                # (重心 − 第 1 頂点)を各断面の紙面へ投影して置く
                # (front_back は (X, Z)・left_right は (Y, Z))。絶対座標(重心
                # そのもの)で置くと第 1 頂点ぶん二重にずれて本体から大きく離れる。
                # primitives は記号を構成する線・円のプリミティブ(紙面ローカル
                # 座標 mm):
