...
PACKAGE_NAME = "vectorworks-plugin-rebar-single"
MODULE_NAME = "vectorworks_plugin_rebar_single"
# pip でインストールする依存ライブラリ (pyproject.toml の dependencies と
# 同期させること)
DEPENDENCIES = ("certifi",)
REPOSITORY = "min-nano/vectorworks-plugin-rebar-single"
...
def _is_python_interpreter(candidate: str) -> bool:
    ...
    バイナリ wheel が VectorWorks 内蔵 Python で使えるよう、
    バージョン (major.minor) の一致も要求する。
    """
...
def _purge_cached_modules(externals: str, *, purge_dependencies: bool = False) -> None:
    ...
    依存ライブラリも破棄する。依存を更新した場合 (初回インストール・修復時)
    のみ True にすること。C 拡張を含む依存は一度ロードされたモジュールを
    sys.modules から除いて再 import すると「reloaded a second time」警告が
    出るため、変更がないなら破棄しない。
    """
...
def _upgrade_if_available() -> None:
    ...
    # 通常の本体更新では依存は変わらないため破棄しない (C 拡張を再 import
    # すると「reloaded a second time」警告が出るのを避けるため)。
